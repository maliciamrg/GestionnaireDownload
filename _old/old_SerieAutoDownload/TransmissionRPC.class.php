...
class TransmissionRPC
{
   ...
   * @param int|array ids A list of transmission torrent ids
   */
  public function renamepath ( $ids ,$path , $name)
  {
    if ( !is_array( $ids ) ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
    $request = array( "ids" => $ids );
  ...
  public function get ( $ids = array(), $fields = array() )  {
    if ( !is_array( $ids ) ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
    if ( count( $fields ) == 0 ) $fields = array( "id", "name", "status", 
		"doneDate", "haveValid", "totalSize" ,"queuePosition" , "isFinished" , "isStalled" , 
"addedDate" , "activityDate" , "dateCreated" , "startDate" , "percentDone" ,
		"files" , "fileStats" ,"peersConnected" ,"peersGettingFromUs" ,"peersSendingToUs" ,
    "hashString");	// Defaults
    $request = array(
      "fields" => $fields,
      "ids" => $ids
   ...
   */
  public function add_file ( $torrent_location, $save_path = '', $Paused ='false', $extra_options = array() )
  {
    $extra_options['download-dir'] = $save_path;
    $extra_options['filename'] = $torrent_location;
    $extra_options['paused'] = $Paused;
   ...
   * @param extra options Optional extra torrent options
   */
  public function add_metainfo ( $torrent_metainfo, $save_path = '', $Paused ='false', $extra_options = array() )
  {
    $extra_options['download-dir'] = $save_path;
  ...
  protected function request( $method, $arguments )
  {
    ...
    if( substr( $stream_meta['wrapper_data']['headers'][0], 9, 3 ) == "401" )
      throw new TransmissionRPCException( "Invalid username/password.", TransmissionRPCException::E_AUTHENTICATION );
    elseif( substr( $stream_meta['wrapper_data']['headers'][0], 9, 3 ) == "409" )
  ...
  public function GetSessionID()
  {
	...
	if (!isset($stream_meta['wrapper_data']['headers'][0])) {	
		$ch = curl_init();
		...
		$r = strpos( $string, 'X-Transmission-Session-Id: ' ) ;
		if ($r > 0){
		  $this->session_id = trim( substr( $string, $r + 27 , 48 ) );
    ...
    elseif( substr( $stream_meta['wrapper_data']['headers'][0], 9, 3 ) == "409" )	// This is what we're hoping to find
    {
      ...
      throw new TransmissionRPCException( "Unexpected response from Transmission RPC: ".$stream_meta['wrapper_data']['headers'][0] );
    }
    return $this->session_id;
  }
   ...
   */
  public function __construct( $url = 'http://192.168.1.104:9091/transmission/rpc', $username = 'admin', $password = 'admin', $return_as_array = false )
  {
    // server URL
    $this->url = $url;
