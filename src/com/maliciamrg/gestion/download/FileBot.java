...
public class FileBot {
	 ...
	 * Rangerfilm.
	 *
	 * @param pathdesfilmaranger
	 *            the pathdesfilmaranger
	 * @param pathdelabibliothequesdesfilm
	 *            the pathdelabibliothequesdesfilm
	...
	public static void rangerfilm(String pathdesfilmaranger, String pathdelabibliothequesdesfilm) throws JSchException, IOException, InterruptedException {
		...
		Ssh.executeAction(Param.props.getProperty("Filebot.launchechaine") + " -script fn:amc --output  \"" + pathdelabibliothequesdesfilm
				+ "\" --log-file amc.log --action move " + "\"" + pathdesfilmaranger + "\" -non-strict " + "--def \"movieFormat="
		...
		/*
		 * Ssh.executeAction(Param.filebotlaunchechaine + " -rename \"" +
		 * pathdesfilmaranger +
		 * "\" --db TheMovieDB --lang en --conflict override --encoding=UTF-8 --format "
		 * + "\"" + pathdelabibliothequesdesfilm +
		 * "/{n.replaceAll(/[:]/,\"\")} ({y})/{n.replaceAll(/[:]/,\"\")} ({y}, {director}) {vf} {af}\""
		 * + " -r -non-strict ");
		 */
	}
	 ...
	 * Rangerserie.
	 *
	 * @param pathdelaseriearanger
	 *            the pathdelaseriearanger
	 * @param pathdelabibliothequesdelaserie
	 *            the pathdelabibliothequesdelaserie
	 * @throws JSchException
	 *             the j sch exception
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	...
	public static ArrayList<String> rangerserie(String pathdelaseriearanger, String pathdelabibliothequesdelaserie, String pathdelabibliothequesdesfilm)
		...
		// String pathdelaseriearangercorriger =
		// (pathdelaseriearanger.substring(0, pathdelaseriearanger.substring(0,
		// pathdelaseriearanger.length()-1).lastIndexOf("/")))+"/" ;
		Ssh.executeAction("rm /mnt/HD/HD_a2/ffp/opt/share/filebot/data/history.xml");
		Ssh.executeAction(Param.props.getProperty("Filebot.launchechaine") + " -clear-cache ");
		ArrayList<String> ret = Ssh.executeAction(Param.props.getProperty("Filebot.launchechaine") + " -script fn:amc --output  \""
				+ pathdelabibliothequesdelaserie + "\" --log-file amc.log --action "/*
																					 * move
																					 * "
																					 */+ "test " + "\"" + pathdelaseriearanger + "\" -non-strict "
				+ "--def \"animeFormat=" + pathdelabibliothequesdelaserie
				+ "/{n}/Saison {s.pad(2)}/{n} S{s.pad(2)}E{es*.pad(2).join('-E')} ep_{absolute*.pad(3).join('_')} {t}\" " + "\"seriesFormat="
				+ pathdelabibliothequesdelaserie + "/{n}/Saison {s.pad(2)}/{n} S{s.pad(2)}E{es*.pad(2).join('-E')} ep_{absolute*.pad(3).join('_')} {t}\" "
	 ...
	 */
	private static String correctiondestination(String src, String dest, String pathdelabibliothequesdelaserie, String pathdelabibliothequesdesfilm)
			throws SQLException, InterruptedException, JSchException, IOException, NumberFormatException {
		if (src.indexOf(pathdelabibliothequesdesfilm) > -1) {
			return dest;
		}
		if (dest.indexOf(pathdelabibliothequesdesfilm) > -1) {
			return dest;
		}
		String code0 = "";
		String code1 = "";
	...
	public static void getsubtitleserie(String pathdelabibliothequesdelaserie) throws JSchException, IOException, InterruptedException {
		// Ssh.executeAction(Param.filebotlaunchechaine +
		// " -get-missing-subtitles \"" + pathdelabibliothequesdelaserie
	 ...
	 *             the interrupted exception
	 */
	public static void maj_liste_episodes(String serie) throws NumberFormatException, SQLException, ParseException, JSchException, IOException,
		...
		for (String lineEp : arrayListEpisode) {
			...
			if (Param.isNumeric(exLineEp[1]) && Param.isNumeric(exLineEp[2]) && Integer.parseInt(exLineEp[2]) > 0) {
				...
				rsFilebot = stmt.executeQuery("SELECT * " + " FROM episodes " + "WHERE " + " serie = \"" + serie + "\"" + " and num_saison = \""
						+ Integer.valueOf(exLineEp[1]) + "\"" + " and num_episodes = \"" + Integer.valueOf(exLineEp[2]) + "\"" + " ");
				rsFilebot.last();
				String nomnettoyer = exLineEp[5].replaceAll("[^a-zA-Z0-9.-]", "_");
				if (rsFilebot.getRow() == 0) {
					stmt.executeUpdate("INSERT INTO episodes " + " ( nom , airdate , serie , num_saison , num_episodes , sequentiel) VALUES " + " (" + " \""
							+ nomnettoyer + "\" ," + " \"" + (new SimpleDateFormat("yyyy-MM-dd")).format(airDate) + "\" ," + " \"" + serie + "\" ," + " \""
							+ Integer.valueOf(exLineEp[1]) + "\" ," + " \"" + Integer.valueOf(exLineEp[2]) + "\" ," + " \"" + Integer.valueOf(exLineEp[3])
							+ "\" " + " )");
				} else {
	...
	private static void mettredatemajserieserie(String serie) throws SQLException {
		Statement stmt = Param.con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
		stmt.executeUpdate("UPDATE series " + " set date_maj_web = \"" + (new SimpleDateFormat("yyyy-MM-dd")).format(Param.dateDuJour()) + "\"" + "WHERE "
				+ " nom = \"" + serie + "\"" + " ");
	}
